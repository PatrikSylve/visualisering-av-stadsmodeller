...
var viewer = new Cesium.Viewer('cesiumContainer', {
    ...
    terrainProvider : new Cesium.CesiumTerrainProvider({
        url: Cesium.IonResource.fromAssetId(69891)
    })
});
...
var tileset = viewer.scene.primitives.add(
    // skeppsbron 77196 (Bridge), 69893
    // gamla hisingsbron 80839
    new Cesium.Cesium3DTileset({ url: Cesium.IonResource.fromAssetId(69893) })
);
...
viewer.scene.primitives.add(
    new Cesium.Cesium3DTileset({
        url: Cesium.IonResource.fromAssetId(75931)
    })
);
*/
...
tileset.tileLoad.addEventListener(function (tile) {
    ...
    for (var i = 0; i < content.featuresLength; ++i) {
        ...
        // var value = feature.getPropertyNames();
        //  console.log(feature.getProperty(value[2]), value);
    }
...
function attributeCol(value, color, selectedAttribute) {
    ...
    tileset.style = new Cesium.Cesium3DTileStyle({
        color: {
            conditions: [
                ["${Coclass-Construction Entity} === `CAE`", "color('lime')"],
...
function test() {
    /** tileset.tileVisible.addEventListener(function(tile) {
         if (tile.content instanceof Cesium.Batched3DModel3DTileContent) {
             console.log('batched', tile.content.getFeature(42));
         }
     });
     **/
    console.log(tileset.tileVisible)
}
...
if (Cesium.PostProcessStageLibrary.isSilhouetteSupported(viewer.scene)) {
    ...
    viewer.screenSpaceEventHandler.setInputAction(function onLeftClick(movement) {
        ...
        for (var i = 0; i < pickedFeature.getPropertyNames().length; i++) {
            selectedEntity.description += '<tr><th>' + i + '</th><td>' + pickedFeature.getProperty(pickedFeature.getPropertyNames()[i]) + '</td></tr>';
        }
...
function pickObjects() {
    //var pickedObjects = viewer.scene.pick(new Cesium.Cartesian2(10.0, 20.0), 1000, 1000);
